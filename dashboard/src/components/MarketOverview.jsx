...
const MarketOverview = ({ events, isDark }) => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-7xl mx-auto">
        ...
        <div className={`${cardClass} md:col-span-3 p-4`}>
          ...
          <p className={`${textBody} text-sm leading-relaxed font-medium`}>
            Mercados operam com cautela aguardando dados de inflação nos EUA.
            Setor de Energia mostra resiliência com alta do petróleo, enquanto
            Tech sofre leve correção. No Brasil, atenção voltada para
            declarações fiscais e movimento do Dólar.
            <span className="text-slate-400 dark:text-slate-500 text-xs ml-2 block mt-1 font-mono">
              Atualizado há 2 min • IA Confidence: 94%
            </span>
          </p>
        </div>
          ...
          >
            ...
            <div className="space-y-2 overflow-y-auto pr-1 custom-scrollbar">
              ...
              {bullish.map((event) => (
                <div
                  key={event.id}
                  className="pl-2 border-l-2 border-green-500 dark:border-green-600"
                >
          ...
          >
            ...
            <div className="space-y-2 overflow-y-auto pr-1 custom-scrollbar">
              ...
              {bearish.map((event) => (
                <div
                  key={event.id}
                  className="pl-2 border-l-2 border-red-500 dark:border-red-600"
                >
                  <div className="flex justify-between items-baseline mb-0.5">
                    <span className="text-[10px] font-mono text-slate-400 dark:text-slate-500">
                      {new Date(event.timestamp).toLocaleTimeString([], {
